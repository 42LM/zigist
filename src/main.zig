...
pub fn timestamp2DateTime(timestamp: i64) DateTime {
    ...
    const sekunden_seit_Mitternacht: u64 = unixtime % SEKUNDEN_PRO_TAG;
    var temp: u64 = 0;
    ...
    const monatu64: u64 = @intCast(monat);
    const tag: u8 = @intCast(tagN - (monatu64 * 153 + 2) / 5 + 1);
    ...
    const stunde: u8 = @intCast(sekunden_seit_Mitternacht / 3600);
    const minute: u8 = @intCast(sekunden_seit_Mitternacht % 3600 / 60);
    const sekunde: u8 = @intCast(sekunden_seit_Mitternacht % 60);
